...
void sl_pwm_set_duty_cycle(sl_pwm_instance_t *pwm, uint16_t percent)
{
  uint32_t top = TIMER_TopGet(pwm->timer);

  // Set compare value
  TIMER_CompareBufSet(pwm->timer, pwm->channel, (top * percent) / COUNT_PWM_MAX);
}

uint16_t sl_pwm_get_duty_cycle(sl_pwm_instance_t *pwm)
{
  uint32_t top = TIMER_TopGet(pwm->timer);
  uint32_t compare = TIMER_CaptureGet(pwm->timer, pwm->channel);

  uint16_t percent = (uint16_t)((compare * COUNT_PWM_MAX) / top);
